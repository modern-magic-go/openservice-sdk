...
func TestMiniProgramAccessToken_PostsSignedQueryAndDecodesResponse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(Result[map[string]string]{
			Code:    0,
			Message: "success",
			Data:    map[string]string{"access_token": "MINI_ACCESS_TOKEN"},
		})
	}))
	defer server.Close()
	...
	resp, err := client.MiniProgram().AccessToken(context.Background())
	require.NoError(t, err)
	assert.Equal(t, "MINI_ACCESS_TOKEN", resp.AccessToken)
}
